...
export default function Repositories() {
  ...
  useEffect(() => {
    api
      ...
      .catch((e) => {
        if (e?.response?.status === 409) setConnected(false);
      })
    ...
    <AppShell>
      ...
      {!loading && !connected && (
        <section className="connect">
          ...
          <button className="btn btn--primary" onClick={startGitHubLink}>
      ...
      {!loading && connected && repos && (
        <>
          ...
          <div className="repolist">
            {shown.length === 0 && (
              <p className="muted">No repositories match your filters.</p>
            )}
            {shown.map((r) => (
              <div className="repo" key={r.fullName}>
                <a
                ...
                >
                  ...
                  {r.description && (
                    <div className="repo__desc">{r.description}</div>
                  )}
                ...
                <button
                  className={
                    "watchbtn" +
                    (watched.has(r.fullName) ? " watchbtn--on" : "")
                  }
                  onClick={() => toggleWatch(r.fullName)}
                  title={
                    watched.has(r.fullName)
                      ? "Auto-review is on — click to turn off"
                      : "Turn on automatic review when PRs open"
                  }
                >
                  {watched.has(r.fullName)
                    ? "Auto-review on"
